...
for (const s of shots) {
  ...
  await p.goto(BASE_URL + route, { waitUntil: "domcontentloaded", timeout: 120000 });
  ...
  const audit = await p.evaluate(() => {
    ...
    return { noAlt, h1, overflow, small: [...new Set(small)].slice(0, 8) };
  });
  console.log(`${route.padEnd(12)} h1=${audit.h1} imgNoAlt=${audit.noAlt} overflow=${audit.overflow} under44=[${audit.small.join(", ")}] errors=${errs.length ? errs.join(" | ") : "none"}`);
  await p.close();
}
await b.close();
